...
# 1. DUMMY SERVER FOR RENDER PORT CHECK
...
# --- RELIABLE API EXTRACTION (COBALT) ---
def extract_images_from_threads(threads_url):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            elif data.get("status") in ["redirect", "stream"]:
                img_urls.append(data.get("url"))
    except Exception as e:
        logging.error(f"Cobalt API Error: {e}")

    # Fallback to direct download of unique extracted URLs
    image_bytes_list = []
    for u in img_urls:
        try:
        ...
        try:
            ...
            if pdf_buffer:
                bot.send_document(
                    ...
                    caption=f"✅ <b>Threads PDF Ready!</b>\nExtracted exactly {len(images)} slide(s)."
                )
        except Exception as e:
